...
var _ = Describe("CF Go Buildpack", func() {
	...
	Context("deploying a Go app with Dynatrace agent with single credentials service", func() {
	...
	Context("deploying a Go app with Dynatrace agent with failing agent download and ignoring errors", func() {
	...
	Context("deploying a Go app with Dynatrace agent with two dynatrace services", func() {
	...
	Context("deploying a Go app with Dynatrace agent with single credentials service and without manifest.json", func() {
		It("checks if Dynatrace injection was successful", func() {
			serviceName := "dynatrace-" + cutlass.RandStringRunes(20) + "-service"
			command := exec.Command("cf", "cups", serviceName, "-p", fmt.Sprintf("'{\"apitoken\":\"secretpaastoken\",\"apiurl\":\"%s\",\"environmentid\":\"envid\"}'", dynatraceAPIURI))
			_, err := command.CombinedOutput()
	...
	Context("deploying a Go app with Dynatrace agent with failing agent download and checking retry", func() {
	...
	Context("deploying a Go app with Dynatrace agent with single credentials service and a redis service", func() {
		It("checks if Dynatrace injection was successful", func() {
			serviceName := "dynatrace-" + cutlass.RandStringRunes(20) + "-service"
			command := exec.Command("cf", "cups", serviceName, "-p", fmt.Sprintf("'{\"apitoken\":\"secretpaastoken\",\"apiurl\":\"%s\",\"environmentid\":\"envid\"}'", dynatraceAPIURI))
			_, err := command.CombinedOutput()
